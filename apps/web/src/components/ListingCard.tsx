...
export function ListingCard({ listing }: { listing: ListingSummary }) {
  ...
  return (
    <Link href={`/listings/${listing.id}`} className="card block hover:shadow-md transition">
      <div className="relative aspect-[4/3] bg-sand-100">
        ...
        {isAuction && (
          <span className="absolute right-2 top-2 rounded-full bg-gold px-3 py-1 text-sm font-bold text-white">
            مزاد
          </span>
        )}
      ...
      <div className="p-4">
        <h3 className="mb-1 text-lg font-bold line-clamp-1">{listing.title}</h3>
