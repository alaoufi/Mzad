...
async function main() {
  ...
  await prisma.healthAttribute.deleteMany();
  ...
  await prisma.auction.update({
    where: { id: auction.id },
    data: { highestBidId: bid2.id },
  });

  console.log('✅ تم! حسابات تجريبية (الجوال): 0500000001 (بائع)، 0500000002 (دلال)، 0500000003 (مشتري)');
  console.log(`   مزاد تجريبي: /auctions/${auction.id}`);
}
